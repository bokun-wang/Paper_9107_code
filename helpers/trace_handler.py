...
class Trace_Handler(object):
    ...
    def get_trace(self):
        return self.time, self.grad_oracles, self.dist_trace, self.fgap_trace, self.max_value, self.alpha_trace, self.iter_trace, self.mbs_trace
    ...
    def update_time(self, seed, time):
        self.time[seed].append(time)

    def update_trace(self, seed, x, f_eval, r_0_dist, r_0_func):
        # if self.problem_type == 'scvx':
        # distance = np.linalg.norm(x - self.x_opt)**2/r_0

        distance = np.linalg.norm(x - self.x_opt) ** 2/r_0_dist
        self.dist_trace[seed].append(distance.item())
        # elif self.problem_type == 'cvx':
        # subopt_gap = (f_eval - self.f_opt)/r_0
        subopt_gap = (f_eval - self.f_opt)/r_0_func
