...
from sklearn.preprocessing import normalize, minmax_scale, maxabs_scale
import os
from loss_functions.utils import safe_sparse_norm
...
if is_normalized:
    # A_ds = minmax_scale(A.toarray(), axis=1)
    # A = sparse.csr_matrix(A_ds)
    A = maxabs_scale(A, axis=1)

# re-ranking the data

all_norms = np.zeros(N)
for i in range(N):
    A_row = A[i]
    all_norms[i] = np.linalg.norm(A_row.A, ord=2)
