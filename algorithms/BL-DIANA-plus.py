...
parser = argparse.ArgumentParser(description='Run DIANA+ (block quantization)')
...
all_Ail = all_dl_sqrt + all_norm_il / (l2 * n_workers)
tilde_T_i1 = 1 + onp.linalg.norm(diag_Li, ord=2) / (l2 * n_workers * onp.sqrt(dim))
tilde_T_iB = onp.sum(all_dl_sqrt * all_Ail) / dim
tilde_delta_iB = dim * tilde_T_iB / (2 * (beta - n_blocks)) + onp.sqrt(
    (dim ** 2) * (tilde_T_iB ** 2) / (4 * (beta - n_blocks) ** 2) + dim * (tilde_T_i1 ** 2) / (beta - n_blocks))
h_final = [tilde_delta_iB / Ail for Ail in all_Ail]
