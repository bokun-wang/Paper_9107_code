...
if problem_type == 'cvx':
    lr0 = 1 / (2 * max_Li * (1 + 2 * omega_sd / n_workers))
else:
    lr0 = 1 / (max_Li * (1 + 2 * omega_sd / n_workers))
...
for seed in seeds_i:
    ...
    for k in range(it_max):
        ...
        for bl in range(n_blocks):
            ...
            hi_k_bl = hi_k[coord_idx]
            n_bit_bl, code_bl, nnz_lst_bl, x_norm_bl, x_sign_bl, Qi_k_bl = standard_dithering(local_g_bl - hi_k_bl,
                                                                                              n_levels, norm_type)
        ...
        timer.start()
        hi_k_next = hi_k + Qi_k / (omega_sd + 1)
        ...
        timer.start()
        h_k_next = h_k + Q_k / (n_workers * (omega_sd + 1))
