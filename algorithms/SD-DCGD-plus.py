...
h_final = onp.sqrt(onp.sum(diag_Li) / diag_Li) / beta
f_h = onp.linalg.norm(diag_Li * h_final, ord=2)

max_f_h = comm.allreduce(f_h, op=MPI.MAX)

if problem_type == 'cvx':
    lr0 = 1 / (2 * (L + 2 * max_f_h / n_workers))
else:
    lr0 = 1 / (L + 2 * max_f_h / n_workers)
...
for seed in seeds_i:
    ...
    for k in range(it_max):
        ...
        timer.start()
        n_bit, code, nnz_lst, x_norm, x_sign, _ = standard_dithering_plus(onp.matmul(mLi_isqrt, local_g), h_final, norm_type)
