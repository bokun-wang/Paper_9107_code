...
parser = argparse.ArgumentParser(description='Run DIANA+ (standard dithering)')
...
sqrt_Aij_pr = [onp.sqrt(1 + (Lij / (n_workers * l2)) ** 2) for Lij in diag_Li]
h_final = onp.sqrt(onp.sum(sqrt_Aij_pr) / sqrt_Aij_pr) / beta
f_h = onp.linalg.norm(diag_Li * h_final, ord=2)

max_f_h = comm.allreduce(f_h, op=MPI.MAX)

omega_sd = onp.minimum(onp.sum(h_final ** 2), onp.sqrt(onp.sum(h_final ** 2)))
    ...
    for k in range(it_max):
        ...
        if rank == 0:
            ...
            for i in range(n_workers):
                ...
                Q_k += onp.matmul(mLi_sqrt_i, norm_i * sign_i * (ell_i * h_final_i))
        else:
            Q_k = None
