...
parser = argparse.ArgumentParser(description='Run DCGD+ (block quant)')
...
tilde_T_i1 = onp.linalg.norm(diag_Li, ord=2) / onp.sqrt(dim)
tilde_T_iB = onp.sum(all_dl_sqrt * all_norm_il) / dim
tilde_delta_iB = dim * tilde_T_iB / (beta - n_blocks) + onp.sqrt(dim / (beta - n_blocks)) * tilde_T_i1
h_final = [tilde_delta_iB / norm_il for norm_il in all_norm_il]
...
if problem_type == 'cvx':
    lr0 = 1 / (2 * (L + 2 * tilde_L_max / n_workers))
else:
    lr0 = 1 / (L + 2 * tilde_L_max / n_workers)
...
for seed in seeds_i:
    ...
    for k in range(it_max):
        ...
        Precond_g = onp.matmul(mLi_isqrt, local_g)
        for bl in range(n_blocks):
            if bl < n_blocks - 1:
                coord_idx = onp.arange(bl * dim_b, (bl + 1) * dim_b)
            else:
